...
namespace GreedyVox.Networked {
    ...
    public class NetworkedCharacterTransformMonitor : NetworkBehaviour {
        ...
        private void OnDestroy () {
            CustomMessagingManager.UnregisterNamedMessageHandler (m_MsgServerTransform);
        ...
        public override void NetworkStart () {
            ...
            m_MsgClientTransform = $"{NetworkObjectId}MsgClientTransform{OwnerClientId}";
            m_MsgServerTransform = $"{NetworkObjectId}MsgServerTransform{OwnerClientId}";
            ...
            if (m_CharacterLocomotion.Platform != null) {
                ...
                if (platform == null) {
                    ...
                    if ((flag & (byte) TransformDirtyFlags.Position) != 0)
                        stream.WriteVector3Packed (m_NetworkPosition);
                    if ((flag & (byte) TransformDirtyFlags.Rotation) != 0)
                        stream.WriteVector3Packed (m_NetworkRotation.eulerAngles);
                }
            } else if (m_Transform != null) {
                // Write flag as dirty
                stream.WriteByte (flag);
